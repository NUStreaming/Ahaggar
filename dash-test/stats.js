...
function computeAverage(resultByDownload, metricToCompute) {
  let total = 0;
  let count = 0;
  for (var key in resultByDownload) {
    if (resultByDownload.hasOwnProperty(key)) { 
        if (metricToCompute in resultByDownload[key]) {
          total += resultByDownload[key][metricToCompute];
          count += 1;
        }
    }
  }
...
module.exports = { computeAverageBitrate, computeAveragePlaybackRate, computeAverage };
